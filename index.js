    ...
    socket.on('message', (message) => {
        try {
            ...
            if (parsedMessage.type === 'join') {
                ...
                if (rooms[roomId]) {
                    for (let client of rooms[roomId].values()) {
                        if (client.readyState === WebSocket.OPEN && client !== socket) {
                            client.send(JSON.stringify({ type: 'message', room: roomId, content: content, sender: sender }));
                        }
                    }
                } else {
                ...
                const roomId = parsedMessage.room;
                const base64Image = parsedMessage.data; // The Base64 encoded image data
                const sender = parsedMessage.sender;

                if (!roomId || !base64Image || !sender) {
                    console.error("Room ID, image data, and sender are required for images");
                    return;
                }
                ...
                if (rooms[roomId]) {
                    for (let client of rooms[roomId].values()) {
                        if (client.readyState === WebSocket.OPEN && client !== socket) {
                            client.send(JSON.stringify({ type: 'image', room: roomId, data: base64Image, sender: sender }));
                        }
                    }
                } else {
                    console.log(`Room ${roomId} does not exist.`);
